...
### 一定要看到下面的效果才继续, 一定不要急(任何一个Master)
### 注意一共 24个 + 所有主机用于ceph硬盘的数量(rook-ceph-osd-x-xxxx)! 数量一定要够
[root@master01 ~]# kubectl get pod -n rook-ceph
NAME                                                              READY   STATUS        RESTARTS   AGE
...
NAME                                         READY   STATUS    RESTARTS   AGE
nginx-curl-ceph-dp-7c8589f9bc-b5rc6          1/1     Running   0          25s

### 进入容器创建文件index.html(任何一个Master)
[root@master01 ~]# kubectl exec -it nginx-curl-ceph-dp-7c8589f9bc-b5rc6 -- /bin/bash
...
### 网页访问https://nginx-ceph.qytangk8s.com/

### 删除pod 和 删除deploy, 文件依然存在

### 如果希望删除文件, 就需要删除pvc(先删除调用pvc的deploy)
...
NAME                                         READY   STATUS    RESTARTS   AGE
nginx-curl-ceph-dp-656cc5fdc7-dtjgr          1/1     Running   0          2m
nginx-curl-ceph-dp-656cc5fdc7-j9vfk          1/1     Running   0          2m
nginx-curl-ceph-dp-656cc5fdc7-wzv9x          1/1     Running   0          2m
